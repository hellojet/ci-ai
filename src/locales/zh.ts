const zh = {
  // 通用
  common: {
    save: '保存',
    saveChanges: '保存修改',
    cancel: '取消',
    create: '创建',
    edit: '编辑',
    delete: '删除',
    confirm: '确认',
    back: '返回',
    search: '搜索',
    upload: '上传',
    export: '导出',
    loading: '加载中...',
    success: '操作成功',
    failed: '操作失败',
    yes: '是',
    no: '否',
    ok: '确定',
    close: '关闭',
    submit: '提交',
    reset: '重置',
    more: '更多',
    noData: '暂无数据',
    total: '共',
    actions: '操作',
    replace: '替换',
    refresh: '刷新',
    generate: '生成',
  },

  // 登录页
  login: {
    subtitle: 'AI 视频全流程自动化创作平台',
    usernamePlaceholder: '用户名',
    passwordPlaceholder: '密码',
    usernameRequired: '请输入用户名',
    passwordRequired: '请输入密码',
    signIn: '登录',
    loginSuccess: '登录成功',
    loginFailed: '登录失败',
    noAccount: '还没有账号？',
    signUp: '立即注册',
  },

  // 注册页
  register: {
    subtitle: '创建账号',
    usernamePlaceholder: '用户名',
    passwordPlaceholder: '密码',
    confirmPasswordPlaceholder: '确认密码',
    usernameRequired: '请输入用户名',
    usernameLength: '用户名长度为 3-64 个字符',
    passwordRequired: '请输入密码',
    passwordLength: '密码长度为 6-128 个字符',
    confirmRequired: '请确认密码',
    passwordMismatch: '两次密码不一致',
    signUp: '注册',
    registerSuccess: '注册成功！请登录',
    registerFailed: '注册失败',
    hasAccount: '已有账号？',
    signIn: '立即登录',
  },

  // 布局 / 导航
  layout: {
    projects: '项目',
    assetLibrary: '资产库',
    settings: '设置',
    userManagement: '用户管理',
    trialRequests: '试用申请',
    logout: '退出登录',
    switchLang: '切换语言',
  },

  // 项目列表
  projectList: {
    title: '项目',
    projectCount: '{count} 个项目',
    newProject: '新建项目',
    noProjects: '暂无项目，创建你的第一个项目吧！',
    createProject: '创建项目',
    loadFailed: '加载项目失败',
    createSuccess: '项目创建成功',
    createFailed: '创建项目失败',
    deleteConfirm: '确认删除该项目？',
    deleteDescription: '所有场景和分镜将被永久删除',
    deleteSuccess: '项目已删除',
    deleteFailed: '删除项目失败',
    createTitle: '新建项目',
    projectName: '项目名称',
    projectNamePlaceholder: '例如：赛博朋克短片',
    projectNameRequired: '请输入项目名称',
    description: '描述',
    descriptionPlaceholder: '简要描述你的项目',
    visualStyle: '视觉风格',
    stylePlaceholder: '选择预设风格',
    imagesPerShot: '每个分镜生成图片数',
    by: '创建者',
  },

  // 项目详情
  projectDetail: {
    notFound: '项目不存在',
    backToProjects: '返回项目列表',
    script: '剧本',
    scriptToggle: '导入剧本',
    scriptToggleCollapse: '收起剧本',
    scenes: '场景',
    shots: '分镜',
    shotsCount: '{count} 个分镜',
    generateImages: '批量生成图片',
    generateVideos: '批量生成视频',
    generateAllImages: '全部生成图片',
    generateAllVideos: '全部生成视频',
    exportProject: '导出项目',
    exportJson: '导出 JSON',
    exportZip: '导出 ZIP',
    batchStarted: '批量{type}生成已启动',
    batchFailed: '批量生成失败',
    exportStarted: '导出已开始',
    exportFailed: '导出失败',
    noLockedImage: '{count} 个分镜未锁定图片，请先锁定图片',
    addScene: '添加场景',
    addShot: '添加分镜',
    sceneName: '场景名称',
    shotName: '分镜名称',
    narration: '旁白',
    dialogue: '对白',
    actionDescription: '动作描述',
    cameraAngle: '镜头角度',
    characters: '角色',
    environment: '场景环境',
    prompt: '提示词',
    status: '状态',
    lockImage: '锁定图片',
    generateScript: '生成剧本',
    parseScript: '拆解剧本',
    generateImage: '生成图片',
    generateVideo: '生成视频',
    generateAudio: '生成音频',
    // 状态翻译
    draft: '草稿',
    in_progress: '进行中',
    completed: '已完成',
    image_generated: '图片已生成',
    video_generated: '视频已生成',
    audio_generated: '音频已生成',
  },

  // 分镜卡片上的生成状态文案
  shotCard: {
    generationStarted: '任务已提交',
    generationFailed: '生成失败',
    queued_image: '排队中…',
    queued_video: '排队中…',
    queued_audio: '排队中…',
    running_image: '生成图片中…',
    running_video: '生成视频中…',
    running_audio: '生成音频中…',
    selectImageModel: '选择图片模型',
    noImageModels: '后端未配置图片模型，请联系管理员在环境变量 AI_IMAGE_MODELS 中添加',
    selectVideoModel: '选择视频模型',
    noVideoModels: '后端未配置视频模型，请联系管理员在环境变量 AI_VIDEO_MODELS 中添加',
    generateWithModel: '使用 {model} 生成',
  },

  // 任务 / 分镜状态徽章
  status: {
    shot: {
      pending: '待处理',
      images_generated: '图片已生成',
      image_locked: '已锁定图片',
      video_generated: '视频已生成',
      completed: '已完成',
    },
    task: {
      pending: '排队中',
      processing: '生成中',
      completed: '生成完成',
      failed: '失败',
      cancelled: '已取消',
    },
  },

  // 资产库
  assets: {
    title: '资产库',
    characters: '角色',
    environments: '场景环境',
    styles: '风格',
    searchPlaceholder: '搜索资产...',
    // 角色
    newCharacter: '新建角色',
    characterName: '角色名称',
    characterNameRequired: '请输入角色名称',
    characterCreated: '角色创建成功',
    characterCreateFailed: '创建角色失败',
    characterDeleted: '角色已删除',
    characterDeleteFailed: '删除角色失败',
    noCharacters: '暂无角色',
    gender: '性别',
    male: '男',
    female: '女',
    age: '年龄',
    appearance: '外观',
    visualPrompt: '视觉提示词',
    views: '视图',
    generateViews: '生成视图',
    seedImage: '种子图片',
    referenceImage: '参考图',
    // 场景环境
    newEnvironment: '新建场景环境',
    environmentName: '环境名称',
    environmentNameRequired: '请输入环境名称',
    environmentCreated: '场景环境创建成功',
    environmentCreateFailed: '创建场景环境失败',
    environmentDeleted: '场景环境已删除',
    environmentDeleteFailed: '删除场景环境失败',
    noEnvironments: '暂无场景环境',
    // 风格
    newStyle: '新建风格',
    styleName: '风格名称',
    styleNameRequired: '请输入风格名称',
    styleCreated: '风格创建成功',
    styleCreateFailed: '创建风格失败',
    styleDeleted: '风格已删除',
    styleDeleteFailed: '删除风格失败',
    noStyles: '暂无风格',
    // 通用
    name: '名称',
    description: '描述',
    prompt: '提示词',
    promptRequired: '请输入提示词',
    tags: '标签',
    deleteConfirm: '确认删除？',
    saveSuccess: '保存成功',
    saveFailed: '保存失败',
    loadFailed: '加载失败',
    backToAssets: '返回资产库',
    count: '{count} 个',
    // 角色详情
    characterInfo: '角色信息',
    characterBrief: '角色简介',
    characterBriefPlaceholder: '输入角色简介...',
    characterNotFound: '角色不存在',
    generateCharacterViews: '生成角色视图',
    visualPromptPlaceholder: '描述角色的视觉特征...',
    voiceProfile: '声音档案',
    voiceSample: '语音样本',
    uploadVoice: '上传音频',
    replaceVoice: '替换音频',
    voiceUploaded: '声音档案上传成功',
    voiceDeleted: '声音档案已删除',
    deleteVoiceConfirm: '确定删除此声音档案？',
    // 场景环境详情
    environmentDetails: '场景环境详情',
    environmentDescPlaceholder: '描述场景环境...',
    environmentNotFound: '场景环境不存在',
    // 风格详情
    styleDetails: '风格详情',
    stylePrompt: '风格提示词',
    stylePromptPlaceholder: '描述风格特征...',
    styleNotFound: '风格不存在',
    // 图片生成
    baseImage: '基础图片',
    referenceViews: '参考视图',
    viewTypes: '视图类型',
    viewDeleted: '视图已删除',
    deleteViewConfirm: '确认删除该视图？',
    deleteViewFailed: '删除视图失败',
    viewGenerationStarted: '视图生成已启动',
    generateViewsFailed: '生成视图失败',
    uploadView: '上传视图',
    uploadViewSuccess: '视图上传成功',
    uploadViewFailed: '视图上传失败',
    viewStatusQueued: '排队中',
    viewStatusGenerating: '生成中',
    viewStatusFailed: '生成失败',
    generateViews: '生成视图',
    useSeedImage: '参考种子图片',
    useSeedImageTip: '开启后，将把角色的种子图片作为参考传给生图模型，生成的视图会更贴合种子图的外观特征。参考图生成约需 2-3 分钟，请耐心等待。',
    seedImageMissingTip: '当前角色还未设置种子图片，请先到角色信息里上传',
    refSeedBadge: '参考',
    refSeedBadgeTip: '本视图基于种子图片生成',
    // 三视图（character turnaround sheet）
    turnaround: '三视图',
    turnaroundTip: '一张合成图里同时展示角色的正面、侧面、背面，作为角色设定参考。每个角色至多保留一张，重新生成会覆盖旧图。',
    generateTurnaround: '生成三视图',
    regenerateTurnaround: '重新生成三视图',
    uploadTurnaround: '上传三视图',
    deleteTurnaroundConfirm: '确认删除三视图？',
    turnaroundGenerationStarted: '三视图生成已启动',
    generateTurnaroundFailed: '生成三视图失败',
    turnaroundEmpty: '尚未生成三视图',
    imageGenerationStarted: '图片生成已启动',
    generateImageFailed: '生成图片失败',
    generationPrompt: '生成提示词',
    generationPromptPlaceholder: '输入图片生成提示词...',
    aiGenerate: 'AI 生成',
    notConfigured: '未配置',
    // 场景资产：参考图 / 视图管理 / 模型下拉
    generateEnvironmentImages: '生成场景图片',
    environmentImages: '场景图片',
    uploadImage: '上传图片',
    uploadImageSuccess: '图片上传成功',
    uploadImageFailed: '图片上传失败',
    deleteImageConfirm: '确认删除该图片？',
    deleteImageSuccess: '图片已删除',
    deleteImageFailed: '删除图片失败',
    seedImage: '种子图片',
    environmentSeedImageTip: '开启后，将把场景的种子图片作为参考传给生图模型，生成的图片会更贴合种子图的空间与氛围。参考图生成约需 2-3 分钟，请耐心等待。',
    environmentSeedMissingTip: '当前场景还未设置种子图片，请先在左侧上传',
    environmentViewTypes: '视角/构图',
    imageModel: '图像模型',
    imageModelPlaceholder: '选择要使用的图像模型',
    noImageModels: '暂无可用图像模型，请在后端 AI_IMAGE_MODELS 中配置',
  },

  // 设置
  settings: {
    title: '系统设置',
    saveAll: '全部保存',
    saveSuccess: '设置已保存',
    saveFailed: '保存设置失败',
    textApi: '文本生成 API',
    imageApi: '图片生成 API',
    videoApi: '视频生成 API',
    audioApi: '音频生成 API',
    apiEndpoint: 'API 接口地址',
    apiEndpointPlaceholder: 'https://api.example.com/v1/...',
    model: '模型名称',
    modelPlaceholder: '例如：deepseek-v4-flash',
    apiKey: 'API 密钥',
    apiKeyPlaceholder: 'sk-...',
    concurrency: '并发数',
    timeout: '超时时间(秒)',
    currentConcurrency: '当前并发数',
    adminOnly: '仅管理员可修改 API 配置，如需修改请联系管理员。',
    // 图像模型清单现在由后端环境变量 AI_IMAGE_MODELS 统一维护
    imageModelsManagedByEnv: '图片模型清单由后端环境变量 AI_IMAGE_MODELS 统一配置，前端在生成画面里下拉选择。',
    availableImageModels: '当前可用模型',
    imageModelsEmpty: '暂无可用模型，请联系运维在 .env 的 AI_IMAGE_MODELS 中配置',
    defaultModelTag: '默认',
  },

  // 管理员
  admin: {
    title: '用户管理',
    userCount: '{count} 个用户',
    user: '用户',
    username: '用户名',
    role: '角色',
    credits: '积分',
    created: '创建时间',
    actions: '操作',
    roleAdmin: '管理员',
    roleUser: '普通用户',
    manageCredits: '管理积分',
    loadFailed: '加载用户列表失败',
    creditModalTitle: '管理积分 — {username}',
    currentBalance: '当前余额',
    amountLabel: '数额（正数=充值，负数=扣减）',
    reason: '原因',
    reasonPlaceholder: '例如：手动充值',
    newBalance: '变更后余额',
    update: '更新',
    creditSuccess: '积分{action}成功',
    creditFailed: '积分更新失败',
    added: '充值',
    deducted: '扣减',
    createUser: '新增用户',
    createSuccess: '用户创建成功',
    createFailed: '用户创建失败',
    usernameRequired: '请输入用户名',
    usernamePlaceholder: '输入用户名',
    password: '密码',
    passwordRequired: '密码至少 6 位',
    passwordPlaceholder: '输入密码（至少 6 位）',
  },

  // 试用申请管理
  trialAdmin: {
    title: '试用申请',
    countLabel: '共 {count} 条',
    searchPlaceholder: '搜索邮箱 / 姓名 / 公司',
    statusAll: '全部状态',
    statusPending: '待处理',
    statusContacted: '已联系',
    statusApproved: '已通过',
    statusRejected: '已拒绝',
    columnId: 'ID',
    columnName: '姓名',
    columnEmail: '邮箱',
    columnCompany: '公司',
    columnStatus: '状态',
    columnCreated: '提交时间',
    columnActions: '操作',
    detailTitle: '申请详情',
    fieldUseCase: '用途',
    fieldIp: '提交 IP',
    fieldUserAgent: 'User Agent',
    fieldNotes: '内部备注',
    notesPlaceholder: '记录沟通进展、决策原因等（仅 admin 可见）',
    save: '保存',
    saveSuccess: '已保存',
    saveFailed: '保存失败',
    deleteConfirm: '确定删除该申请？',
    deleteSuccess: '已删除',
    deleteFailed: '删除失败',
    loadFailed: '加载失败',
    empty: '暂无试用申请',
  },

  // 落地页
  landing: {
    header: {
      signIn: '登录',
      requestTrial: '申请试用',
      enterApp: '进入工作台',
    },
    hero: {
      titleLine1: '让一段文字',
      titleLine2: '长成一部影像',
      subtitle: '资产驱动 · 角色一致 · 一键成片',
      ctaPrimary: '立即登录',
      ctaSecondary: '申请试用',
    },
    features: {
      sectionTitle: '为什么选择 CI.AI',
      sectionSubtitle: '把"创意有，但产能低"变成"想到就能做出来"',
      items: {
        consistency: {
          title: '资产驱动一致性',
          desc: '角色 / 环境 / 风格定义一次，全片复用，AI 视觉永不跳脱',
        },
        breakdown: {
          title: 'AI 剧本智能拆分',
          desc: '一段文案，自动拆成镜头脚本与场景结构',
        },
        models: {
          title: '多模型矩阵',
          desc: '图像 / 视频 / 语音模型自由切换，按需选最合适的',
        },
        batch: {
          title: '一键批量成片',
          desc: '图 → 视频 → 音频 → ZIP 打包，全程异步',
        },
      },
    },
    workflow: {
      sectionTitle: '从文案到成片，只需四步',
      sectionSubtitle: '每一步都对应平台里的一个工作区，可以来回迭代',
      steps: {
        assets: {
          title: '定义资产',
          desc: '建立角色 / 环境 / 风格库，作为创作的视觉基石',
        },
        breakdown: {
          title: '剧本拆分',
          desc: 'AI 把剧本拆为场景与镜头，自动关联到资产',
        },
        generate: {
          title: '批量生成',
          desc: '图像 → 视频 → 音频，按分镜异步生产',
        },
        export: {
          title: '导出成片',
          desc: '一键打包为 ZIP，包含全部素材与元数据',
        },
      },
    },
    showcase: {
      sectionTitle: '走进 CI.AI 工作区',
      sectionSubtitle: '六大模块共同支撑你的创作流程',
      items: {
        characters: '角色库',
        breakdown: '剧本拆分',
        editor: '镜头编辑器',
        batch: '批量生成进度',
        asset: '资产详情',
        export: '项目导出',
      },
    },
    cta: {
      title: '准备好让 AI 帮你创作下一部影像？',
      subtitle: '注册试用入口仅向受邀团队开放，请留下联系方式与我们对接',
      ctaPrimary: '立即登录',
      ctaSecondary: '申请试用',
    },
    footer: {
      copyright: '© 2026 CI.AI · AI 视频创作平台',
      contact: '联系：hello@ci-ai.example.com',
    },
    trialModal: {
      title: '申请试用',
      subtitle: '留下联系方式，我们将在 24 小时内联系您',
      name: '姓名',
      namePlaceholder: '怎么称呼您？',
      nameRequired: '请输入姓名',
      email: '邮箱',
      emailPlaceholder: 'name@company.com',
      emailRequired: '请输入邮箱',
      emailInvalid: '邮箱格式不正确',
      company: '公司 / 团队（选填）',
      companyPlaceholder: '所在公司或团队名',
      useCase: '想用 CI.AI 做什么？（选填）',
      useCasePlaceholder: '例如：拍一支 60 秒短视频广告 / 做一个 5 集动画短剧…',
      submit: '提交申请',
      cancel: '取消',
      successMsg: '已收到，我们会在 24 小时内联系您',
      duplicateMsg: '该邮箱 24 小时内已申请过，请稍后再试',
      tooManyMsg: '请求过于频繁，请稍后再试',
      errorMsg: '提交失败，请稍后重试',
    },
  },
};

export default zh;
export type Locale = typeof zh;
